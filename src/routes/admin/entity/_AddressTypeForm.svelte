...
<script>
  ...
  S.bind$(save_('address_type', rowIdx), ([d]) => { isSaving = false; if (d.ok) {  er = ""; dp("successSave", {rowIdx, d});  } else { er = d.error; } }, 1); 

  async function save() { isSaving = true; S.trigger([[ save_('address_type', rowIdx), [form, [form.id]] ]]); }
