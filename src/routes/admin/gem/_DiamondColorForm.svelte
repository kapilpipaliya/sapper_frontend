...
<script>
  ...
  if (item.length) { form = makeObject(hs, item) }; S.bind$(save_('d_color', rowIdx), ([d]) => { isSaving = false; if (d.ok) {  er = ""; dp("successSave", {rowIdx, d});  } else { er = d.error; } }, 1);

  async function save() { isSaving = true; S.trigger([[ save_('d_color', rowIdx), form ]]); }
