...
<script>
  ...
  onMount(async ()=>{
    ...
    fns.push(["user", "user_register", 0]); S.bind$(fns.i(-1), ([d]) => {isSaving = false; if (d.ok) {  er = ""; formSave = true; dp("successSave", { d }); } else { er = d.error; } }, 1)
  ...
  const save = async() => {isSaving = true; S.trigger([[ ["user", "user_register", 0], form ]]); }
  const clearError = () => { er = ""; }
