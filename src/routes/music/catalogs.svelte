...
export async function preload({query}, session) {
  ...
  const getTableData = async(filterSettings=[]) => {
    ...
    await new Promise((resolve, reject) => {
        const batch1 = []
        let r0 = false, r1 = false, r2 = false, r3 = false, r4 = false, r5 = false
        const myResolve = () => {
          if(r0 && r1 && r2 && r3 && r4 && r5) resolve(1)
        }
    ...
    return {menu, h, data, count, user_title, user_account_type};
  }

  const {menu, h, data, count, user_title, user_account_type} = await getTableData([]);
