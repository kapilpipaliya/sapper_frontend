<script>
  ...
  S.bind$(node_save_("menu_node", rowIdx), (d) => { isSaving = false; if (d.ok) {  er = ""; dp("successSave", { rowIdx, d }); } else { er = d.error; } });
  
  async function save() { isSaving = true; S.trigger(node_save_("menu_node", rowIdx), form); }
