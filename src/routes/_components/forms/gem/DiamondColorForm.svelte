...
<script>
  ...
  if (item.length) { form = makeObject(hs, item) }; S.bind$(m_save_('d_color', rowIdx), (d) => { isSaving = false; if (d.ok) {  er = ""; dp("successSave", {rowIdx, d});  } else { er = d.error; } });

  async function save() { isSaving = true; S.trigger(m_save_('d_color', rowIdx), form); }
