...
<script>
  ...
  S.bind$(s_save_("global_setting", rowIdx), (d) => { isSaving = false; if (d.ok) {  er = ""; dp("successSave", { rowIdx, d });  } else { er = d.error; } });
    
  //S.bind_(s_all(`metal`, rowIdx), (d) => { metals = d; form.metal_id = item.length ? form['metal_id'] : (metals[0] ? metals[0][0] : 0) }, []);

  async function save() { isSaving = true; S.trigger(s_save_("global_setting", rowIdx), form); }
