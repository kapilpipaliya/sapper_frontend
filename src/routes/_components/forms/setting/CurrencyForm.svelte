...
<script>
  ...
  S.bind$(s_save_("currency", rowIdx), (d) => { isSaving = false; if (d.ok) {  er = ""; dp("successSave", { rowIdx, d });  } else { er = d.error; } });
    
  async function save() { isSaving = true; S.trigger(s_save_("currency", rowIdx), form); }
