<script>
  ...
  S.bind$(f_save_('mfg_txn', rowIdx), (d) => { isSaving = false; if (d.ok) {  er = ""; dp("successSave", {rowIdx, d});  } else { er = d.error; } }); 

  async function save() { isSaving = true; S.trigger(f_save_('mfg_txn', rowIdx), form); }
