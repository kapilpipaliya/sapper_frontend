<script>
  ...
  import { f_all, e_all, f_save_, makeObject } from "../../../_modules/functions.js";
  ...
  fns.push(f_all("department", rowIdx)); S.bind_(fns.i(-1), (d) => { department_id = d; form.department_id = item.length ? form["department_id"] : (department_id[0] ? department_id[0][0] : 0) }, [[]]);
  fns.push(e_all("entity", rowIdx)); S.bind_(fns.i(-1), (d) => { employee_id = d; form.employee_id = item.length ? form["employee_id"] : (employee_id[0] ? employee_id[0][0] : 0) }, [[null, "=8"]]);
