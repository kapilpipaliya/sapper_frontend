...
<script>
  ...
  S.bind$(m_save_('tone', rowIdx), (d) => { isSaving = false; if (d.ok) {  er = ""; dp("successSave", {rowIdx, d});  } else { er = d.error; } });

  async function save() { isSaving = true; S.trigger(m_save_('tone', rowIdx), form); }
