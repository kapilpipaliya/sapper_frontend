...
<script>
  ...
  S.bind$(e_save_('entity_type', rowIdx), (d) => { isSaving = false; if (d.ok) {  er = ""; dp("successSave", {rowIdx, d});  } else { er = d.error; } }); 

  async function save() { isSaving = true; S.trigger(e_save_('entity_type', rowIdx), form); }
