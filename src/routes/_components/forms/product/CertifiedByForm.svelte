<script>
  ...
  import { all, p_save_, makeObject } from "../../../_modules/functions.js";
  ...
  export let hs = [];
    ...
    let isSaving = false;
  let er = "";
  let form = { slug: "", name: "", title: "", description: "" }
  let parent = []

  if (item.length) { form = makeObject(hs, item) }; 
  S.bind$(p_save_("certified_by", rowIdx), (d) => { isSaving = false; if (d.ok) {  er = ""; dp("successSave", { rowIdx, d }); } else { er = d.error; } });
  
  async function save() { isSaving = true; S.trigger(p_save_("certified_by", rowIdx), form); }
...
<form on:submit|preventDefault={save} >
  ...
  <label><span>Slug</span><input type="text" bind:value={form.slug} /></label>
	...
	<label><span>Title</span><input type="text" bind:value={form.title} /></label>
