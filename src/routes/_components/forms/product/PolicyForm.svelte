<script>
  ...
  S.bind$(p_save_("policy", rowIdx), (d) => { isSaving = false; if (d.ok) {  er = ""; dp("successSave", { rowIdx, d }); } else { er = d.error; } });
  
  async function save() { isSaving = true; S.trigger(p_save_("policy", rowIdx), form); }
